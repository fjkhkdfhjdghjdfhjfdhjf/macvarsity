...
module.exports = function (app) {
    ...
    let storage = multer.diskStorage({
        destination: (_req, _file, cb) => cb(null, path.join(__dirname + "../../../uploads/matches/")), // where the files are being stored
    ...
    app.use('/api/uploads/matches',
        upload.array('files'), (req, _res) => {
            const { method } = req;
            if (method === 'POST') {
                for (const file of req.files) {
                    _res.json({ "name": file.path.replace(path.join(__dirname + "../../../uploads/matches/"), "") })
                }
